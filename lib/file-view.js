...
export default class FileView {
  constructor (parentView, indexInParentView, archivePath, entry) {
    ...
    if (addIconToElement) {
      const fullPath = path.join(archivePath, this.entry.path)
      const iconDisposable = addIconToElement(this.name, fullPath)
      this.disposables.add(iconDisposable)
    } else {
      let typeClass = IconServices.get('file-icons').iconClassForPath(this.entry.path, 'archive-view') || []
      if (!Array.isArray(typeClass) && typeClass) {
        typeClass = typeClass.toString().split(/\s+/g)
      }
      nameClasses.push(...typeClass)
    }
    this.name.classList.add(...nameClasses)
